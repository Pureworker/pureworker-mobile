...
const PLUGIN_SETTINGS: any = {
  common: [
    // Geolocation
    {
      ...
      defaultValue: 1,
      values: [
        {label: 'Geofences only', value: 0},
        {label: 'Location + Geofences', value: 1},
      ],
    },
    {
      ...
      values: [
        {
          label: 'NAVIGATION',
          value: BackgroundGeolocation.DESIRED_ACCURACY_NAVIGATION,
        },
        ...
        {
          label: 'MINIMUM',
          value: BackgroundGeolocation.DESIRED_ACCURACY_VERY_LOW,
        },
  ...
  ios: [
    ...
    },
    {
      ...
      values: [
        ...
        {
          label: 'AUTOMOTIVE_NAVIGATION',
          value: BackgroundGeolocation.ACTIVITY_TYPE_AUTOMOTIVE_NAVIGATION,
        },
        {label: 'FITNESS', value: BackgroundGeolocation.ACTIVITY_TYPE_FITNESS},
        {
          label: 'OTHER_NAVIGATION',
          value: BackgroundGeolocation.ACTIVITY_TYPE_OTHER_NAVIGATION,
        },
      ],
...
export default class SettingsService {
  ...
  constructor(props: any) {
    ...
    const items = []
      .concat(PLUGIN_SETTINGS.common)
      .concat(PLUGIN_SETTINGS[this.platform]);
  ...
  getApplicationSettings(group: any) {
    if (group !== undefined) {
      let settings = [];
      return APP_SETTINGS.filter((setting: any) => {
  ...
  */
  ...
  async applyTestConfig() {
    ...
    await BackgroundGeolocation.reset({
      ...
      backgroundPermissionRationale: {
        title:
          "Allow Pureworker to access this device's location even when closed or not in use.",
        message:
          'Pureworker collects location data to enable recording your trips to work and calculate distance-travelled.',
  ...
  _saveState() {
    AsyncStorage.setItem(
      ...
      JSON.stringify(this.applicationState, null),
    );
  }
}
